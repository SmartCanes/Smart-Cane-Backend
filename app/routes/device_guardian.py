    ...
    try:
        ...
        if not device_id and not device_serial:
            return error_response(
                "`device_id` or `device_serial_number` is required", 400
            )
        ...
        existing = DeviceGuardian.query.filter_by(
        ...
        return success_response(
            data={"id": link.id}, message="Guardian assigned to device", status_code=201
        )
...
def list_assignments(guardian):
    try:
            ...
            data = [
                {
                    ...
                    "device_serial_number": (
                        l.device.device_serial_number if l.device else None
                    ),
                    "assigned_at": l.assigned_at.isoformat() if l.assigned_at else None,
                }
                for l in links
        ...
        return error_response(
            "Provide `device_id` or `guardian_id` as query parameter", 400
        )
